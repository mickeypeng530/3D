...
- **病人服 = TSL colorNode 高度帶**(模型空間 y,disfigure rest=T-pose):上衣 torsoBand y 0.70-1.60(下緣已降到接短褲)× sideIn |x|<0.34 × 圓領口挖空;短袖 sleeve(armX |x| 0.28-0.43 沿手臂 = 收到 proximal humerus;橫截面用**徑向** armR²=(y-1.46)²+z² 圓管,**不要用水平 y 帶**——下垂手臂會被 y 帶斜切成斜向 hem,徑向才前後對稱垂直);短褲 y 0.66-1.08 全寬;點點花紋 sin 網格。gown 色 `#7791ba`、膚色 `#c9b29a`。disfigure 的 `dress()` 在此版是 silent no-op 別浪費時間
