...
> 最後更新:2026-06-15(swimmer 板上光野投影+陰影、肩部光、十字線寬可調,build sw18)

## sw18:swimmer 光影做成「示範圖」風 + 十字線寬可調
- **範圍閘只管身體**:`gateRegion = onSurface ? 1 : bodyGate`。承光面(牆/板/檯)不受閘限制 → **X 光板照常顯示光野矩形+十字+後頸投影陰影**(occl `lit` 機制:焦點→板的射線穿過頸 occl 橢球=陰影)。swimmer 設 `surfaceField:1` 打開板上光野。
- **肩部光**:身體閘放大 `paintGate {z:-1.37, r:0.13, ry:0.20}`(涵蓋後頸+肩),`r` 仍窄到排除舉起的手臂(在 -z 板側)。
- **十字線寬可調**:`uCrossW`(半寬 m,預設 0.011)取代寫死的 0.004/0.011;UI「十字線寬 (mm)」滑桿(1-30mm)。
- 使用者微調值:`tube {x:-0.94, h:1.5, pitch:88, fieldW:0.16, fieldH:0.38}`、SID 102。**十字耳後位置/肩部範圍仍可再用滑桿微調**。
